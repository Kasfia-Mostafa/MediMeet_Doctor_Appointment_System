...
export default function DoctorDetail() {
  ...
  useEffect(() => {
    const fetchDoctor = async () => {
      try {
        ...
        setNewSlot((prev) => ({
          ...prev,
          day: data.availableDays?.[0] || "monday",
        }));
  ...
  const addSlot = () => {
    ...
    if (availableDays.length === 0)
      return toast.error("Select at least one available day first");
    setForm((prev) => ({
      ...prev,
        ...
        >
          ...
          >
            ...
            >
              ...
              <strong>{form.name}</strong> from the system? This action cannot
              be undone and all associated records will be deleted.
            </p>
        ...
        <div className="card mb-lg" style={{ padding: "32px" }}>
          ...
          >
            ...
            <h3 style={{ margin: 0 }}>
              ...
              {form.timeSlots?.filter((ts) => availableDays.includes(ts.day))
                .length || 0}
              )
            </h3>
          </div>
          ...
          <div className="flex flex-wrap gap-sm mb-lg">
            {[
              ...
              <button
                key={day}
                type="button"
                className={`btn btn-sm ${availableDays.includes(day) ? "btn-primary" : "btn-secondary"}`}
          ...
          {availableDays.length === 0 ? (
            ...
            >
              <HiOutlineClock
                style={{ fontSize: "28px", opacity: 0.4, marginBottom: "8px" }}
              />
              <p style={{ margin: 0, fontSize: "14px" }}>
                Select available days above to configure time slots
              </p>
            </div>
          ) : (
            <>
              ...
              {form.timeSlots.filter((ts) => availableDays.includes(ts.day))
                ...
                <div className="table-wrapper mb-lg">
                  <table>
                    ...
                    <tbody>
                      {form.timeSlots
                        .filter((ts) => availableDays.includes(ts.day))
                        .map((ts, i) => (
                          <tr key={i}>
                            ...
                            <td>
                              <button
                                ...
                                onClick={() =>
                                  removeSlot(form.timeSlots.indexOf(ts))
                                }
                              >
                                Remove
                              </button>
                ...
                <div
                  className="mt-lg p-lg text-center"
                  style={{
                ...
                >
                  ...
                  >
                    No scheduling slots configured yet.
                  </p>
                </div>
              )}
            </>
